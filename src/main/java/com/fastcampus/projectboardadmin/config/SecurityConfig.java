...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		return http.authorizeHttpRequests(auth -> auth
				.requestMatchers(PathRequest.toStaticResources().atCommonLocations()).permitAll() // 정적리소스
	...
	/**
	 * UserDetailsService가 하나의 기능만을 구현하는 인터페이스이기에 람다식으로 작성
	 * @param adminAccountService 어드민 유저 서비스
	 * @return 유저
	 */
	@Bean
	public UserDetailsService userDetailsService(AdminAccountService adminAccountService) {
		return username -> adminAccountService.searchUser(username)
