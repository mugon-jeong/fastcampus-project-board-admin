...
class ArticleManagementServiceTest {
	...
	@EnableConfigurationProperties(ProjectProperties.class) // 특정 프로퍼티 읽어오도록 설정
	@AutoConfigureWebClient(registerRestTemplate = true) // 자동으로 설정한 rest template bean 가져와 사용
	@RestClientTest(ArticleManagementService.class) // rest client mocking(테스트 대상)
	@Nested // 중첩 테스트 선언
	class RestTemplateTest {

		private final ArticleManagementService sut;
		private final ProjectProperties projectProperties;
		private final MockRestServiceServer server; // 특정 api 접근하는 행위 mocking
		...
		void givenArticleId_whenCallingArticleApi_thenReturnsArticle() throws Exception {
			...
			server
				.expect(requestTo(
					projectProperties.board().url() + "/api/articles/" + articleId + "?projection=withUserAccount"))
				.andRespond(withSuccess(
